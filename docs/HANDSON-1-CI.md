...
> [!TIP]アクションをコミットハッシュで固定する</summary>
>
...
2. 変更をコミット
    ...
    任意のコミットメッセージを入力して「コミット」ボタンをクリック  
...
3. GitHub へプッシュ
    ...
    「Branchの発行」をクリック（リモートが複数表示される場合は、自分のGitHubアカウントのリポジトリを選択する）  
    ![gitプッシュ1](images/git-push-1.png)

    コミットグラフに反映されていることを確認  
    ![gitプッシュ2](images/git-push-2.png)

4. GitHub 上で PR を作成

    「Pull requests」>「New pull request」
    ![PR1](images/pull-request-1.png)

    対象のブランチを設定してから「Create pull request」をクリック（リポジトリ選択が必要な場合は、自分のアカウントのリポジトリを選択する）  
    ![PR2](images/pull-request-2.png)

    タイトルや説明は特に変えなくていいので「Create pull request」をクリック
    ![PR3](images/pull-request-3.png)

5. PR の「Checks」で CI が失敗することを確認する

    ![failed checks](images/failed-checks.png)

    > [!TIP]
    > Checksがエラーの場合はマージできないようにGitHubを設定することが可能です

6. テストを修正して再度 PR を更新する

    `tests/calculator.test.js`の17~20行目にわざと失敗するテストケースがあるため削除してからコミット＆プッシュ  
    ![failed-testcase](images/failed-testcase.png)

7. 自動で再実行されるので CI が成功することを確認する

    ![succeeded checks](images/succeeded-checks.png)
